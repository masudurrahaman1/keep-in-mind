...
export default function UserDetails() {
  ...
  if (error || !user) {
    return (
      <div className="max-w-2xl mx-auto mt-20 p-10 glass rounded-[32px] text-center border-error/20">
        ...
        <Link to="/admin/users" className="inline-flex items-center gap-2 text-primary font-bold hover:gap-3 transition-all">
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-24 font-sans px-4 md:px-0">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <h1 className="text-3xl md:text-display-metrics gradient-text mb-1 truncate">{user.name}</h1>
            <p className="text-[10px] md:text-xs font-medium text-on-surface-variant opacity-60 flex items-center gap-2 truncate">
               <Shield className="w-3.5 h-3.5 text-primary shrink-0" />
               <span className="truncate">Identity Index: {user._id}</span>
            </p>
          </div>
        </div>
        <div className="flex gap-4">
           <button 
            ...
            className="h-12 md:h-14 px-6 glass rounded-2xl text-error font-bold hover:bg-error/10 transition-all flex items-center gap-3 w-full md:w-auto justify-center"
           >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.section 
          ...
          className="glass p-6 md:p-8 rounded-[28px] md:rounded-[32px] flex flex-col items-center text-center relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full blur-3xl -mr-16 -mt-16" />
          <div className="relative mb-6">
            {user.avatar ? (
              <img src={user.avatar} className="w-24 h-24 md:w-32 md:h-32 rounded-[24px] md:rounded-3xl object-cover border-2 border-outline-variant shadow-2xl" />
            ) : (
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-[24px] md:rounded-3xl bg-primary/10 text-primary flex items-center justify-center text-3xl md:text-4xl font-bold border border-primary/20">
                {user.name.charAt(0)}
              </div>
            )}
            <div className={cn(
              "absolute -bottom-2 -right-2 w-7 h-7 md:w-8 md:h-8 rounded-full border-4 border-surface shadow-lg flex items-center justify-center",
              user.isVerified ? 'bg-secondary' : 'bg-on-surface-variant'
            )}>
              <Shield className="w-3.5 h-3.5 md:w-4 md:h-4 text-on-primary" />
            </div>
          </div>
          <h2 className="text-xl md:text-2xl font-bold text-on-surface mb-1 truncate w-full">{user.name}</h2>
          <span className="text-[9px] md:text-[10px] font-bold text-primary uppercase tracking-[0.2em] mb-6">{user.authProvider} Authorized</span>
          ...
          <div className="w-full space-y-4 pt-6 border-t border-outline-variant/30">
            <div className="flex items-center gap-4 p-4 glass rounded-2xl bg-surface-container/50">
               ...
               <div className="text-left min-w-0">
                  <p className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest opacity-40">Identity Address</p>
            ...
            <div className="flex items-center gap-4 p-4 glass rounded-2xl bg-surface-container/50">
               ...
               <div className="text-left">
                  <p className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest opacity-40">Onboarding Date</p>
        ...
        <div className="lg:col-span-2 space-y-8">
           ...
           <motion.section 
            ...
            className="glass p-6 md:p-8 rounded-[28px] md:rounded-[32px]"
           >
              <div className="flex justify-between items-center mb-8">
                 <h3 className="text-lg md:text-xl font-bold text-on-surface flex items-center gap-3">
              ...
              {user.recentMedia && user.recentMedia.length > 0 ? (
                   ...
                   {user.recentMedia.map((item: any) => (
                     <div key={item._id} className="p-4 glass rounded-2xl flex items-center justify-between group hover:bg-surface-container/50 transition-colors">
                        <div className="flex items-center gap-4 min-w-0">
                           ...
                           <div className="min-w-0">
                              <p className="text-sm font-bold text-on-surface truncate">{item.fileName || "Untitled Asset"}</p>
                ...
                <div className="p-10 md:p-12 text-center glass rounded-2xl border-dashed border-outline-variant/30">
                   <p className="text-on-surface-variant text-sm font-medium">No recent ecosystem activity recorded.</p>
                </div>
              )}
           </motion.section>
        </div>
